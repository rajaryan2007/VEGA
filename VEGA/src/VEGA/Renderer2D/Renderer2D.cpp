...
namespace VEGA {
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const Ref<Texture2D>& texture, f32 tiling_factor , const glm::vec4& tintColor)
	{
	...
	void Renderer2D::DrawQuad(const glm::vec2& position, const glm::vec2& size, const Ref<Texture2D>& texture, f32 tiling_factor , const glm::vec4& tintColor )
	{
		VG_PROFILE_FUNCTION();
		DrawQuad({ position.x ,position.y,0 }, size, texture,tiling_factor,tintColor);
	}

	void Renderer2D::DrawQuad(const glm::vec2& position, const glm::vec2& size, const Ref<SubTexture2D>& Subtexture, f32 tiling_factor /*= 1.0f*/, const glm::vec4& tintColor /*= glm::vec4(1.0f)*/)
	{
		DrawQuad({ position.x,position.y ,0 }, size, Subtexture, tiling_factor,tintColor);
	}
	...
	void Renderer2D::DrawQuad(const glm::mat4& transform, const Ref<SubTexture2D>& texture, f32 tiling_factor /*= 1.0f*/, const glm::vec4& tintColor /*= glm::vec4(1.0f)*/)
	{
		VG_PROFILE_FUNCTION();
		VG_PROFILE_FUNCTION();
		...
		if (textureIndex == 0)
		{
			...
			textureIndex = s_Data.TextureSlotIndex;
			//s_Data.TextureSlots[s_Data.TextureSlotIndex] = texture;
            // Fix for assignment error in Renderer2D.cpp
            // Replace this line:
            // s_Data.TextureSlots[s_Data.TextureSlotIndex] = texture;
	...
	void Renderer2D::DrawRotatedQuad(const glm::vec3& position, const glm::vec2& size, f32 rotation, Ref<Texture2D>& texture, f32 tiling_factor , const glm::vec4& tintColor )
	{
		VG_PROFILE_FUNCTION();
	...
	void Renderer2D::DrawRotatedQuad(const glm::vec3& position, const glm::vec2& size, f32 rotation, Ref<SubTexture2D>& Subtexture, f32 tiling_factor /*= 1.0f*/, const glm::vec4& tintColor /*= glm::vec4(1.0f)*/)
	{
		constexpr size_t quadVertexCount = 4;
		constexpr glm::vec4 color = { 1.0f,1.0f,1.0f,1.0f };
		const glm::vec2* textureCoords = Subtexture->GetTexCoords();
	...
	void Renderer2D::DrawRotatedQuad(const glm::vec2& position, const glm::vec2& size, f32 rotation, Ref<SubTexture2D>& Subtexture, f32 tiling_factor /*= 1.0f*/, const glm::vec4& tintColor /*= glm::vec4(1.0f)*/)
	{
		DrawRotatedQuad({ position.x,position.y,0 }, size, rotation, Subtexture, tiling_factor, tintColor);
	}
