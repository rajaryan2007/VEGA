...
namespace UHE
{
    ...
    namespace Utils {
        ...
        static GLenum UHEToGLFrameBufferTextureFormat(FrameBufferTextureFromat format)
        {
            switch (format)
            {
        ...
        static GLenum UHEToGLFrameBufferTextureFormatAttachment(FrameBufferTextureFromat format)
        {
            switch (format)
            {
    ...
    void OpenGLFrameBuffer::Invalidate()
    {
        ...
        {
			VG_CORE_ASSERT(m_colorAttachments.size() <= 4, "UHE only supports 4 color attachments");
